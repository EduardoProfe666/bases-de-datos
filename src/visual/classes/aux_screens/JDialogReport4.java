...
public class JDialogReport4 extends JDialogGeneral {
	private static final long serialVersionUID = 1L;
	private JRadioButton fina;
	private JRadioButton annual;

	public JDialogReport4(ColorScheme e, JFrame padre) {
		super("Reporte de Promedios", e, padre); 
		...
		fina = new JRadioButton("Final");
		fina.setFont(new Font("Roboto Medium", Font.PLAIN, 17));
		fina.setBounds(284, 61, 70, 23);
		panelContenedor.add(fina);
		btnGroup.add(fina);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if(annual.isSelected()) {
						ServicesLocator.getReportServices().loadReport4A();
						dispose();
					}
					else {
						ServicesLocator.getReportServices().loadReport4F();
						dispose();
					}
